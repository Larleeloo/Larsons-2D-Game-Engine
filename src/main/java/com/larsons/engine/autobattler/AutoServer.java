...
/**
 * The authoritative auto-battler server, hosted exactly like the engine's
 * world server ({@link com.larsons.engine.net.GameServer}): bind a port,
 * players connect to {@code ip:port} — 2 to 10 of them, plus host-added bots.
 * All game state lives in an {@link AutoGame} owned by the tick thread;
 * connections only decode requests onto a queue and write queued replies, so
 * a slow client can never stall the round timer or a running battle.
 */
public final class AutoServer implements AutoGame.Sink {

    /** Game logic tick rate (Hz); combat snapshots go out at ~15 Hz. */
    ...
    public AutoServer(AutoGame.Config config) {
        this.game = new AutoGame(config, this);
    }
    ...
    public synchronized void start(int port) throws IOException {
        ...
        running = true;
        acceptThread = daemon("auto-accept", this::acceptLoop);
        tickThread = daemon("auto-tick", this::tickLoop);
        acceptThread.start();
        tickThread.start();
        log("Hosting auto-battler on port " + getPort());
    }
    ...
    public synchronized void stop() {
        ...
        log("Auto-battler server stopped");
    }
    ...
    private void processJoins() {
        ...
        while ((conn = pendingJoins.poll()) != null) {
            ...
            AutoPlayer p = game.addPlayer(conn.requestedName, false);
            if (p == null) {
                String reason = game.phase() != AutoGame.Phase.LOBBY
                        ? "Game already started" : "Game is full (10 players max)";
                conn.sendNow(Protocol.encode(AutoProto.error(reason)));
                conn.markClosed();
                continue;
            }
            conn.playerId = p.id;
            conn.send(Protocol.encode(AutoProto.welcome(p.id, TICK_RATE)));
            game.sendFullState(p.id);
            toAll(AutoProto.info(p.name + " joined the lobby"));
    ...
    private void processDisconnects() {
        ...
        for (Connection c : connections) {
            ...
            if (c.closed) {
                ...
                if (c.playerId > 0) {
                    AutoPlayer p = game.byId(c.playerId);
                    game.removePlayer(c.playerId);
                    if (p != null) {
                        toAll(AutoProto.info(p.name + " left"));
    ...
    private void processRequests() {
        ...
        while ((req = pendingRequests.poll()) != null) {
            ...
            switch (AutoProto.type(msg)) {
                case "start" -> {
                    if (!game.start(conn.playerId) && game.phase() == AutoGame.Phase.LOBBY) {
                        toPlayer(conn.playerId, AutoProto.info(
                                "Need at least " + AutoGame.MIN_PLAYERS
                                        + " players (add a bot?)"));
                    }
                }
                case "addbot" -> {
                    if (!game.addBot(conn.playerId)) {
                        toPlayer(conn.playerId, AutoProto.info("Can't add a bot now"));
                    }
                }
    ...
    private final class Connection {
        ...
        void startThreads() {
            daemon("auto-read-" + socket.getPort(), this::readLoop).start();
            daemon("auto-write-" + socket.getPort(), this::writeLoop).start();
        }
            ...
            try {
                ...
                while (!closed && (line = in.readLine()) != null) {
                    ...
                    switch (AutoProto.type(msg)) {
                        ...
                        case "ping" -> send(Protocol.encode(AutoProto.pong(
        ...
        void handleJoin(Map<String, Object> msg) {
            ...
            if (version != AutoProto.VERSION) {
                sendNow(Protocol.encode(AutoProto.error(
                        "Incompatible auto-battler version " + version
                                + " (server is " + AutoProto.VERSION + ")")));
                markClosed();
                return;
            }
    ...
    private static void log(String msg) {
        System.out.println("[auto-server] " + msg);
    }
}
